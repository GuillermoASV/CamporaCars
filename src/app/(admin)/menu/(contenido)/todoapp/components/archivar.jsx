...
import { Archive, TimerOff, UserRoundPen, NotebookText, Tag } from 'lucide-react';
...
const DialogoArchivar = ({ tarea, alCerrar, alConfirmar }) => {
  if (!tarea) return;
  ...
  return (
    <Dialog open={true} onOpenChange={alCerrar}>
      <DialogContent
        className={`${tarea.descripcion.length > 270 ? 'h-45' : 'h-160'}max-h-3/4 min-h-1/4`}
      >
        <DialogHeader>
          <DialogTitle className={`relative mb-2 text-center`}>
            <Archive className="absolute -top-1 left-0 overflow-auto text-orange-500" />
            Confirmar archivado de la tarea
          </DialogTitle>
          <DialogDescription className={'pb-2 text-center'}>
            ¿Estas seguro que quieres <span className="font-bold text-orange-500">archivar</span> la
            tarea seleccionada?
          </DialogDescription>
        </DialogHeader>
        <div
        ...
        >
          ...
          <div className="mt-1 flex">
            <span className="mt-1.5 mr-2 flex size-8 items-center justify-center rounded-full bg-gradient-to-br from-blue-500 to-blue-600 px-2 py-1 text-blue-300">
          ...
          <div className="mt-1 flex">
            <span className="size-8items-center mt-1.5 mr-2 flex justify-center rounded-full bg-gradient-to-br from-violet-500 to-violet-600 px-2 py-1 text-gray-300">
        ...
        <DialogFooter className={'flex flex-row justify-between'}>
          ...
          <InteractiveHoverButton variant="delete" className={'cursor-pointer'} onClick={archivar}>
            Confirmar
...
export default DialogoArchivar;
